...
class PPO_continuous():
    ...
    def update(self, replay_buffer, total_steps):
        batch_s, batch_a, batch_a_log, batch_r, batch_s_, batch_done = replay_buffer.sample()  # Get training data
        adv = []
        gae = 0
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                ratios = torch.exp(a_logprob_now.sum(1, keepdim=True) - batch_a_log[index].sum(1, keepdim=True))  # shape(mini_batch_size X 1)
...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.action_log = np.zeros((args.batch_size, args.action_dim))
    ...
    def sample(self):
        ...
        self.count  = 0
        self.reset() # 释放缓存
        return batch_s, batch_a, batch_a_log, batch_r, batch_s_, batch_done
...
def train(args, agent, env):
    ...
    for episode in range(args.max_episodes):
        ...
        for step in range(args.max_steps):
            action, action_log = agent.choose_action(state)  # Action and the corresponding log probability
            next_state, reward, cost, done,  terminated, info = env.step(action, True)
            ep_reward += reward
            ep_cost += cost

            replay_buffer.store(state, action, action_log, reward, next_state, done)
            state = next_state
            total_steps += 1
            env.render()
...
if __name__ == '__main__':
    ...
    args.log_dir = "logs/{}/{}/PPO_level{}_{}".format(args.env_name, args.agent_name, level, args.date)
