...
class ReplayBuffer:
    ...
    def store(self, state, action, reward, next_state, done, a_safe, CBFs):
    ...
    def sample_tensors(self):
        batch_s      = torch.tensor(self.state, dtype=torch.float64)
        batch_a      = torch.tensor(self.action, dtype=torch.float64)
        batch_a_safe = torch.tensor(self.a_safe, dtype=torch.float64)
        batch_r      = torch.tensor(self.reward, dtype=torch.float64)
        batch_s_     = torch.tensor(self.next_state, dtype=torch.float64)
        batch_done   = torch.tensor(self.done, dtype=torch.float64)
        batch_cbfs   = torch.tensor(self.cbfs, dtype=torch.float64)
        self.count   = 0
        self.reset() 
        return batch_s, batch_a, batch_r, batch_s_, batch_done, batch_a_safe, batch_cbfs
...
class TRPO:
    def __init__(self, args):
        ...
        if self.adaptive_lambda:
            ...
            self.lambda_optimizer = torch.optim.Adam([self.log_lambda], lr=3e-4)
        else:
            self.lam_lag = 10.0
    ...
    def update(self, memory):
        states, actions, rewards, s_, masks, a_safe, cbfs = memory.sample_tensors()
        ...
        def get_loss(volatile=False, use_lagrangian=False):
            if volatile:
                with torch.no_grad():
                    action_means, action_log_stds, action_stds = self.actor(Variable(states))
            else:
            ...
            if use_lagrangian:
                self.violation = torch.clamp(cbfs, min=-10.0, max=1.0)
                loss_safe = F.mse_loss(action_means, a_safe)
                action_loss = action_loss + self.lam_lag * loss_safe
                self.log['loss_safe'] = loss_safe.detach().cpu().flatten()
                self.log['violation_mean'] = self.violation.mean().detach().cpu().flatten()
        ...
        loss = get_loss(use_lagrangian=True)
        ...
        if self.use_lagrangian:
            if self.adaptive_lambda:
                penalty = torch.mean(self.violation)
...
def train(args, env, agent):
    ...
    for episode in range(args.max_episodes):
        ...
        for step in range(args.max_steps):
            ...
            memory.store(state, action, reward, next_state, mask, a_safe, CBFs)
            steps_this_iter += 1
            state = next_state
...
if __name__=='__main__':
    ...
    args.log_dir = "logs/{}/{}/TRPOC_level{}_{}".format(args.env_name, args.agent_name, level, args.date)
