...
class ReplayBuffer(object):
    ...
    def store(self, state, action, r, next_state, dw):
...
class SAC(object):
    ...
    def choose_action(self, state, deterministic=False, with_logprob=False):
    ...
    def update(self, replay_buffer):
        ...
        with torch.no_grad():
            ...
            TD_target = batch_r + self.gamma * (1 - batch_dw) * (torch.min(target_Q1, target_Q2) - self.alpha * log_pi_)
        ...
        # Compute actor loss
        action, log_pi = self.actor(batch_s)
        Q1, Q2 = self.critic(batch_s, action)
        Q = torch.min(Q1, Q2)
        ...
        # Update alpha
        if self.adaptive_alpha: # We learn log_alpha instead of alpha to ensure that alpha=exp(log_alpha)>0
